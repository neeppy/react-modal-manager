...
const ContentComponent = ({
  ...
  <div>
    ...
    <button data-testid="closeBtn" onClick={close}>
      Close
    </button>
  </div>
);
...
const ConfirmationComponent = ({
  ...
  text: string;
  close: (isConfirmed?: boolean, data?: unknown) => void;
}) => (
  <div>
    <div data-testid="dialogText">{text}</div>
    <button
      data-testid="resolveBtn"
      onClick={() => close(true, { foo: 'bee' })}
    >
      OK
    </button>
    <button data-testid="closeBtn" onClick={() => close(false)}>
      Close
    </button>
  </div>
);
...
describe('ModalContainer', () => {
  beforeEach(() => {
    render(<ModalContainer data-testid="modal-container" store={store} />);
  });
